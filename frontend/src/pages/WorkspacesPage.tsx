...
export function WorkspacesPage() {
  ...
  const loadMembers = async (ws: Workspace) => {
    setSelectedWs(ws); setMembersLoading(true)
    try {
  ...
  const createWs = async () => {
    ...
    try {
      ...
      setShowCreate(false); setWsName(''); setWsDesc(''); load()
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <Header
        title="Workspaces"
        actions={<Button size="sm" onClick={() => setShowCreate(true)}><Plus className="w-4 h-4" /> New workspace</Button>}
      />
              ...
              {workspaces.map((ws) => (
                <button
                  ...
                  className={clsx(
                    ...
                    selectedWs?.id === ws.id ? 'border-brand-600 shadow-soft' : 'border-gray-800 hover:border-brand-700'
                  )}
                >
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6">
            <div className="flex items-center justify-between mb-5 gap-2">
              <div className="flex items-center gap-2 min-w-0">
                <button
                  ...
                  className="p-1.5 -ml-1.5 text-gray-400 hover:text-gray-900 lg:hidden shrink-0"
            ...
            {membersLoading ? (
              ...
              <div className="space-y-2">
                {members.map((m) => (
                  <div key={m.user_id} className="flex items-center gap-3 surface p-4">
                    ...
                    {m.role !== 'owner' && (
                      <button onClick={() => removeMember(m.user_id)} className="p-1.5 text-gray-600 hover:text-red-400 rounded-lg transition-colors ml-1">
