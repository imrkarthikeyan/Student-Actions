...
function StatusBadge({ meeting }) {
    if (meeting.status === 'live') {
        return (<span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-brand-600 text-white animate-glow">
        <Radio className="w-3 h-3"/> Live
      </span>);
    }
    return (<span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-gray-800 text-gray-300 border border-gray-600">
...
function CreateMeetingModal({ open, onClose, onCreated }) {
    ...
    return (<Modal open={open} onClose={onClose} title="New meeting">
      <div className="flex flex-col gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => setMode('now')} className={clsx('flex-1 px-3 py-2 rounded-lg text-sm font-medium border transition-all', mode === 'now' ? 'bg-brand-600 border-brand-600 text-white' : 'border-gray-700 text-gray-400 hover:text-gray-200')}>
            Start now
          </button>
          <button onClick={() => setMode('schedule')} className={clsx('flex-1 px-3 py-2 rounded-lg text-sm font-medium border transition-all', mode === 'schedule' ? 'bg-brand-600 border-brand-600 text-white' : 'border-gray-700 text-gray-400 hover:text-gray-200')}>
            Schedule for later
          </button>
        </div>

        {mode === 'schedule' && (<Input label="Date & time" type="datetime-local" value={scheduledAt} onChange={(e) => setScheduledAt(e.target.value)} min={new Date().toISOString().slice(0, 16)}/>)}
...
function CreatedMeetingModal({ meeting, onClose }) {
    ...
    return (<Modal open={!!meeting} onClose={onClose} title="Meeting ready">
      <div className="flex flex-col gap-4">
...
export function MeetingsPage() {
    ...
    return (<div className="flex flex-col min-h-screen">
      <Header title="Meetings" actions={<Button size="sm" onClick={() => setShowCreate(true)}>
      ...
      <div className="flex-1 p-4 sm:p-6">
        ...
        {loading ? (<div className="flex justify-center py-16"><Spinner /></div>) : meetings.length === 0 ? (<div className="text-center py-20">
          ...
          </div>) : (<div className="max-w-3xl space-y-3">
            {meetings.map((m) => (<div key={m.id} className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 surface surface-hover p-4">
                ...
                <div className="flex items-center gap-2 shrink-0 sm:ml-auto">
                  <button onClick={() => {
                    ...
                    toast.success('Invite link copied');
                }} className="p-2 text-gray-500 hover:text-gray-100 hover:bg-gray-800 transition-colors shrink-0" title="Copy invite link">
                    <Copy className="w-4 h-4"/>
                  </button>
      ...
      <CreateMeetingModal open={showCreate} onClose={() => setShowCreate(false)} onCreated={(m) => {
            setShowCreate(false);
            setCreated(m);
            load();
        }}/>
