...
function VideoTile({ stream, name, isLocal, muted, cameraOff }) {
    ...
    return (<div className="relative bg-gray-900 rounded-xl overflow-hidden aspect-video flex items-center justify-center border border-gray-800">
...
export function MeetingRoomPage() {
    ...
    if (notFound) {
        return (<div className="min-h-screen bg-gray-950 relative flex flex-col items-center justify-center text-center px-4">
    ...
    if (!joined) {
        return (<div className="min-h-screen bg-gray-950 relative flex flex-col items-center justify-center px-4 gap-6">
        <div className="ambient-bg"/>
        ...
        <div className="relative z-10 w-full max-w-md aspect-video bg-gray-900 rounded-xl overflow-hidden flex items-center justify-center border border-gray-800">
          {previewStream ? (<video autoPlay muted playsInline ref={(el) => { if (el)
                el.srcObject = previewStream; }} className="w-full h-full object-cover"/>) : (<Spinner />)}
        </div>
        <input value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && join()} placeholder="Your name" className="relative z-10 w-full max-w-md bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-center text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-brand-500"/>
    ...
    return (<div className="min-h-screen bg-gray-950 relative flex flex-col">
      <div className="ambient-bg"/>
      <div className="relative z-10 flex flex-col flex-1 min-h-screen">
        ...
        {call.error && (<div className="bg-red-500/10 border-b border-red-500/30 text-red-400 text-sm text-center py-2 shrink-0">
            {call.error}
          </div>)}

        <div className="flex-1 p-3 sm:p-6 grid gap-3 sm:gap-4 content-center" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
        ...
        <div className="flex items-center justify-center gap-3 py-5 border-t border-gray-800 shrink-0">
          <button onClick={call.toggleMic} className={clsx('p-3 rounded-full transition-colors', call.micOn ? 'bg-gray-800 text-gray-200 hover:bg-gray-700' : 'bg-red-600 text-white hover:bg-red-500')} title={call.micOn ? 'Mute' : 'Unmute'}>
            {call.micOn ? <Mic className="w-5 h-5"/> : <MicOff className="w-5 h-5"/>}
          </button>
          <button onClick={call.toggleCamera} className={clsx('p-3 rounded-full transition-colors', call.cameraOn ? 'bg-gray-800 text-gray-200 hover:bg-gray-700' : 'bg-red-600 text-white hover:bg-red-500')} title={call.cameraOn ? 'Turn off camera' : 'Turn on camera'}>
            {call.cameraOn ? <VideoIcon className="w-5 h-5"/> : <VideoOff className="w-5 h-5"/>}
          </button>
          <button onClick={copyLink} className="p-3 rounded-full bg-gray-800 text-gray-200 hover:bg-gray-700 transition-colors" title="Copy invite link">
            <Copy className="w-5 h-5"/>
          </button>
          <button onClick={leave} className="p-3 px-6 rounded-full bg-red-600 text-white hover:bg-red-500 transition-colors" title="Leave call">
