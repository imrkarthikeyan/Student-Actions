...
export function useMeetingCall(roomCode: string, displayName: string, enabled: boolean) {
  ...
  const closePeer = useCallback((sid: string) => {
  ...
  const createPeerConnection = useCallback((socket: Socket, peerSid: string, peerName: string) => {
  ...
  useEffect(() => {
    ...
    async function start() {
      ...
      socket.on('meeting:peers', async ({ peers: existing }: { peers: { sid: string; name: string }[] }) => {
      ...
      socket.on('meeting:peer_joined', ({ sid, name }: { sid: string; name: string }) => {
        namesRef.current[sid] = name
        setPeers((prev) => ({ ...prev, [sid]: { sid, name, stream: null } }))
      })

      socket.on('meeting:peer_left', ({ sid }: { sid: string }) => closePeer(sid))

      socket.on('meeting:signal', async ({ sid, signal }: { sid: string; signal: SignalPayload }) => {
        let pc = pcsRef.current[sid]
        if (signal.type === 'offer' && signal.sdp) {
          if (!pc) pc = createPeerConnection(socket, sid, namesRef.current[sid] ?? 'Guest')
