...
export const useClipboardStore = create((set, get) => ({
    ...
    updateItemInList: (item) => set((s) => ({ items: s.items.map((i) => (i.id === item.id ? item : i)) })),
    removeItemFromList: (id) => set((s) => ({ items: s.items.filter((i) => i.id !== id), total: s.total - 1 })),
}));
