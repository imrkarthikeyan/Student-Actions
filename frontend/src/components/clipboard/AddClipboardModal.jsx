...
export function AddClipboardModal({ open, onClose }) {
    ...
    return (<Modal open={open} onClose={onClose} title="Add Clipboard Item" size="lg">
      {/* Tabs */}
      <div className="flex gap-2 mb-5">
        {['manual', 'upload'].map((t) => (<button key={t} onClick={() => setTab(t)} className={clsx('px-4 py-2 rounded-lg text-sm font-medium transition-all capitalize', tab === t ? 'bg-brand-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-gray-200')}>
      ...
      {tab === 'manual' ? (<div className="space-y-4">
          ...
          <div className="flex gap-2">
            {TYPES.map(({ type: t, label, icon }) => (<button key={t} onClick={() => setType(t)} className={clsx('flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm transition-all', type === t ? 'bg-brand-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-gray-200')}>
                {icon} {label}
              </button>))}
          </div>

          <Input label="Title (optional)" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Auto-generated if empty"/>
          <Textarea label="Content" value={content} onChange={(e) => setContent(e.target.value)} placeholder={type === 'url' ? 'https://…' : type === 'code' ? '// paste code here' : 'Paste or type content…'} rows={6} className={type === 'code' ? 'font-mono' : ''}/>
        ...
        </div>) : (<div {...getRootProps()} className={clsx('border border-dashed rounded-xl p-12 text-center cursor-pointer transition-all', isDragActive ? 'border-brand-500 bg-brand-500/10' : 'border-gray-700 hover:border-gray-600')}>
          <input {...getInputProps()}/>
