...
export function ClipboardWidget() {
  const [items, setItems] = useState<ClipboardItem[] | null>(null)
  ...
  return (
    <FeatureCard icon={<Clipboard className="w-4 h-4" />} title="Clipboard" to="/clipboard">
      {items === null ? (
        <div className="flex-1 flex items-center justify-center py-6"><Spinner /></div>
      ) : items.length === 0 ? (
        <p className="text-sm text-gray-500 text-center py-6">Nothing synced yet</p>
      ) : (
