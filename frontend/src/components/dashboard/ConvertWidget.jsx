...
export function ConvertWidget() {
    ...
    return (<FeatureCard icon={<RefreshCw className="w-4 h-4"/>} title="Quick Convert" to="/convert">
      {result ? (<div className="flex flex-col items-center gap-3 py-4">
          ...
          {file ? (<div className="flex items-center gap-2 bg-gray-800/60 border border-gray-700 rounded-lg px-3 py-2 text-sm">
              <span className="flex-1 min-w-0 truncate text-gray-200">{file.name}</span>
              <select value={targetFormat} onChange={(e) => setTargetFormat(e.target.value)} className="bg-gray-900 border border-gray-700 rounded px-1.5 py-1 text-xs text-gray-100">
                {targets.map((t) => <option key={t} value={t}>.{t.toUpperCase()}</option>)}
              </select>
            </div>) : (<label className="flex flex-col items-center justify-center gap-1.5 border border-dashed border-gray-700 hover:border-brand-500 rounded-lg px-3 py-6 cursor-pointer transition-colors">
