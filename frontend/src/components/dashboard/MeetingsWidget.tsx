...
export function MeetingsWidget() {
  ...
  return (
    <FeatureCard icon={<Video className="w-4 h-4" />} title="Meetings" to="/meetings">
      {meetings === null ? (
        ...
        <div className="flex flex-col gap-3">
          ...
          {meetings.length === 0 ? (
            <p className="text-sm text-gray-500 text-center py-4">No meetings yet</p>
          ) : (
              ...
              {meetings.slice(0, 3).map((m) => (
                <div key={m.id} className="flex items-center gap-2.5 text-sm">
                  <span
                    className={clsx(
                      'shrink-0',
                      m.status === 'live' ? 'text-brand-500' : 'text-gray-500'
                    )}
                  >
