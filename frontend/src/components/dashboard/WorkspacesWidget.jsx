...
export function WorkspacesWidget() {
    ...
    return (<FeatureCard icon={<Users className="w-4 h-4"/>} title="Workspaces" to="/workspaces">
      {workspaces === null ? (<div className="flex-1 flex items-center justify-center py-6"><Spinner /></div>) : workspaces.length === 0 ? (<p className="text-sm text-gray-500 text-center py-6">No workspaces yet</p>) : (<div className="space-y-2">
          {workspaces.slice(0, 4).map((ws) => (<div key={ws.id} className="flex items-center gap-2.5 text-sm">
